...
%% equality constraint main equation
for c = 1:N-tmin+1
    %initiation
    supp_mat = zeros(7,size_theta+N+N+(N-tmin+1)); 
...
% theta columns
    supp_mat(:,1:9) = [zeros(1,9);[[eye(3);zeros(3)],[zeros(6,3)],[zeros(3);eye(3)]]]
     ...
     % z2: 
    supp_mat(1:4,c+N+size_theta:c+N+size_theta+tmin-1)=flip(eye(4));
    
    %coef_vec
    coef_vec = [1 1 1 1 -u2(c+tmin-2) -u2(c+tmin-3) -u2(c+tmin-4)]';

    ineqPolySys{c}.noTerms = 7;
    ineqPolySys{c}.typeCone = -1; %equality
    ineqPolySys{c}.dimVar = size_theta + 3*N - tmin + 1;
...
%% partial output 1 (z1)
for c = N-tmin+2:2*(N-tmin+1)
    i = c - (N-tmin+1) %dummy counter
    %initiation
    supp_mat = zeros(7,size_theta+N+N+(N-tmin+1)); 
...
%% partial output 2 (z2)
for c = 2*(N-tmin+1)+1:3*(N-tmin+1)
    i = c - 2*(N-tmin+1) %dummy counter
      %initiation
    supp_mat = zeros(4,size_theta+N+N+(N-tmin+1)); 
% a1 a2 a3 b11 b21 b31 b12 b22 b32,[z1(1)..z1(N)],[z2(1)..z2(N)],[eta(4)..eta(N)]
    ...
    supp_mat(2,i+size_theta+tmin-1) = 1;
    % z2 columns:

    supp_mat(3,i+size_theta+N+tmin-1) = 1;
    % eta columns:
    supp_mat(4,i+size_theta+N+N)=1;

    %coef vector
    coef_vec = [y_tilde(i+tmin-1) -1 -1 -1]';
...
% lower bound
lbd = [-1e10*ones(size_theta+2*N,1);-Delta_eta(2)*ones(N-tmin+1,1)];
% upper bound
ubd = -lbd;
...
for i = 1:size_theta
    objPoly.noTerms = 1;
    objPoly.degree = 1;
    objPoly.dimVar = size_theta + 3*N - tmin + 1;
    objPoly.typeCone = 1;

    supp = zeros(1, size_theta + 3*N - tmin + 1);
    supp(i) = 1;
