...
delta_e = 10
A_EE_inf = [A_EE;-A_EE];
b_OE = [y_LS_EE - delta_e;-y_LS_EE + delta_e]
F = eye(5);
PUI = zeros(5,2);
...
for j = 1:2
    sign = -1*sign;
    for i = 1:5
        PUI(i,j) = F(i,:)*linprog(sign*F(i,:)',A_EE_inf,b_OE);
    end
end
...
delta_e = 10;
A_OE_inf = [A_OE;-A_OE];
b_OE = [y_LS_OE - delta_e;-y_LS_OE + delta_e]
...
for j = 1:2
    sign = -1*sign;
    for i = 1:5
        PUI(i,j) = F(i,:)*linprog(sign*F(i,:)',A_OE_inf,b_OE);
    end
end
